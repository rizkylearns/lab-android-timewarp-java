 ...
 * <code>if(RootShell.isSuAvailable()) { RootShell.runCommand("reboot"); }</code>
 * <p/><p/>
 * To get process output as a String:<p/>
 * <code>if(RootShell.isSuAvailable()) { String date = RootShell.getProcessOutput("date"); }</code>
 * <p/><p/>
 * To run command with standard shell (no root permissions):
 * <code>RootShell.setShell("sh");</code><p/>
 * <code>RootShell.runCommand("date");</code>
 * <p/><p/>
 *
 * taken from @author serge 
 * http://code.google.com/p/market-enabler/source/browse/branches/MarketAccess/src/ru/org/amip/MarketAccess/utils/ShellInterface.java
 */
